...
const { LengthError, ContentError } = require('app-errors')

const { env: { MONGODB_URL_TEST } } = process

describe('retrieveUser()', () => {
    before(() => mongoose.connect(MONGODB_URL_TEST, { useUnifiedTopology: true, useNewUrlParser: true, useCreateIndex: true }))
    ...
    describe('when user id is wrong', () => {
        ...
        describe('when user id length is not 24', () => {
            ...
            it('should fail on user id length different from 24', () => {
                expect(() => retrieveUser(userId, () => { })).to.throw(LengthError, `id length ${userId.length} and should be 12/24`)
            })
        })
    })
